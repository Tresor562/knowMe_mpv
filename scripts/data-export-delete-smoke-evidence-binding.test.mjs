...
const cliPath = fileURLToPath(new URL('./data-export-delete-smoke-evidence-binding.mjs', import.meta.url));
...
test('CLI creates data lifecycle evidence from a regular retained artifact', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'knowme-data-lifecycle-binding-'));
  try {
    const artifactPath = join(dir, 'data-export-delete.json');
    ...
    assert.equal(item.id, 'data_export_delete_validation');
...
test('CLI rejects a symlinked data lifecycle retained artifact before JSON ingestion', async (t) => {
  if (process.platform === 'win32') {
    t.skip('Symlink creation is not reliably available on Windows CI without elevated privileges.');
    return;
  }

  const dir = await mkdtemp(join(tmpdir(), 'knowme-data-lifecycle-binding-symlink-'));
  try {
    const targetPath = join(dir, 'data-export-delete-target.json');
    const artifactPath = join(dir, 'data-export-delete-link.json');
    const outputPath = join(dir, 'item.json');
