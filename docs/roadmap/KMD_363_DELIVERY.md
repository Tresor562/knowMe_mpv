...
14. **CI #1371/#1374 — explicit production media limit and preflight correction.** The graph probe localized the first missing production value to `media/media-upload-policy.js`: production intentionally has no implicit upload-size default. Both the graph probe and real API boot now receive `MEDIA_UPLOAD_MAX_BYTES=26214400` (25 MiB). The first dedicated preflight overmatched the non-production fallback; it was corrected to inspect only the production guard while still requiring production to throw when the setting is absent.
15. **CI #1375 — graph load passes; Nest provider creation exposed the account-security encryption requirement.** Supply-chain policy, frozen installation, production audit, Prisma generation/migrations/drift, monorepo build/tests, API image build, compiled entrypoint proof, native dependency smokes, the application-graph probe, non-root identity and health metadata all passed. The real container then exited `1` during `nest-application-create`. Its logs show many modules initialized but no `SecurityModule`, `AuthModule` or `AccountModule`. `SecurityCryptoService` intentionally requires a 32-byte `ACCOUNT_SECURITY_ENCRYPTION_KEY` in production and refuses the development JWT-derived fallback there. Canonical CI now supplies an explicit fixed **CI-only** 32-byte key to both the graph probe and real API boot. A dedicated preflight requires both bindings and separately proves that production still throws when the key is absent and never falls back to `JWT_SECRET`. `ACCOUNT_SECURITY_ENCRYPTION_KEY` is also allowlisted as a configuration key for bounded startup classification; its value is never logged.
...
The runtime proof binds the GitHub Actions PostgreSQL service and test-only JWT material plus explicit CI release identity, one-instance rate limiting, trusted proxy, HTTP timeouts, reserved HTTPS CORS origin, `MEDIA_UPLOAD_MAX_BYTES=26214400`, a fixed CI-only 32-byte `ACCOUNT_SECURITY_ENCRYPTION_KEY`, and synthetic S3 configuration. The media upload value is the repository policy's bounded 25 MiB maximum and is supplied explicitly because production intentionally has no implicit upload-limit default. The account-security key exists only to exercise the production encryption-key validator; production remains fail-closed when that key is absent and does not derive it from `JWT_SECRET`. These values are not production credentials, domains, storage proof, horizontal-scale evidence or deployment approval.

## Tests and merge gates

KMD-363 is complete only when CI for the **exact current PR head** passes:

1. supply-chain and package/lockfile preflights, including dedicated production media and account-security configuration preflights;
...
8. bounded application-graph load probe with `app-module-load-ok` under production guards, including explicit bounded media and account-security configuration;
...
Revert the KMD-363 commits. This restores KMD-362 behavior where CI builds the images and verifies healthcheck metadata without proving that the production commands, native runtime dependencies and application graph actually start successfully. Reverting the #1375 correction specifically removes the CI-only `ACCOUNT_SECURITY_ENCRYPTION_KEY` bindings, bounded startup classification entry and dedicated preflight; it must not be replaced by enabling the development JWT-derived fallback in production or by weakening `SecurityCryptoService`. No persistent-data rollback is required.
