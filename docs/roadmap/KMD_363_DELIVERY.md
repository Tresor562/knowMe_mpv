...
14. **CI #1371 — graph probe localized a missing production media limit.** The exact candidate again passed supply-chain policy, frozen install, production audit threshold, Prisma generation/migrations/drift, monorepo build/tests, API image construction, deterministic entrypoint proof and both native-runtime smokes. The new graph probe then failed with the bounded marker `loading:media/media-upload-policy.js`. `MediaController` evaluates `resolveMediaUploadMaxBytes()` while constructing its `FileInterceptor`, and the policy intentionally throws in `NODE_ENV=production` unless `MEDIA_UPLOAD_MAX_BYTES` is explicitly configured. The failure was therefore a missing CI production configuration value, not a reason to relax the policy. Both the graph probe and the real API boot now receive the bounded canonical maximum `26214400` bytes (25 MiB). A dedicated preflight requires both occurrences and separately locks the fail-closed production policy so the CI fix cannot regress into an implicit production default.
...
The runtime proof binds the GitHub Actions PostgreSQL service and test-only JWT material plus explicit CI release identity, one-instance rate limiting, trusted proxy, HTTP timeouts, reserved HTTPS CORS origin, `MEDIA_UPLOAD_MAX_BYTES=26214400`, and synthetic S3 configuration. The media upload value is the repository policy's bounded 25 MiB maximum and is supplied explicitly because production intentionally has no implicit upload-limit default. These values exist solely to exercise production validators. They are not production credentials, domains, storage proof, horizontal-scale evidence or deployment approval.

## Tests and merge gates

KMD-363 is complete only when CI for the **exact current PR head** passes:

1. supply-chain and package/lockfile preflights, including the dedicated production media-configuration preflight;
...
8. bounded application-graph load probe with `app-module-load-ok` under production guards, including an explicit bounded media upload limit;
...
Revert the KMD-363 commits. This restores KMD-362 behavior where CI builds the images and verifies healthcheck metadata without proving that the production commands, native runtime dependencies and application graph actually start successfully. Reverting the #1371 correction specifically removes the explicit CI-only `MEDIA_UPLOAD_MAX_BYTES` binding and its preflight; it must not be replaced by weakening the production media policy. No persistent-data rollback is required.
