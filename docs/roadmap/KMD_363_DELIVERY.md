...
- Emit a bounded `unowned-exit` phase only when a non-zero direct process termination bypasses the normal bootstrap rejection diagnostic.
...
- Repository preflight tests prevent removing the boot proof, making the wait unbounded, changing the runtime to development/test, regressing to a leaf-only API build, omitting required CI production configuration, bypassing the guarded launcher, restoring top-level runtime imports or static `AppModule` loading, restoring Nest internal abort behavior, logging raw startup exceptions, removing the bounded non-zero direct-exit phase fallback, silently removing failure-diagnostic artifact persistence, or hiding named external GitHub Actions from the immutable-pin audit.
...
4. bounded pre-bootstrap launcher plus bootstrap-owned runtime/application-module loading, bounded direct-exit phase reporting and secret-safe diagnostics;
