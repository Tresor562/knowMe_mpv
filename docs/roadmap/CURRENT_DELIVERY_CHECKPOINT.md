...
- Live GitHub has progressed through merged `KMD-374`.
...
- KMD-375 is the current active delivery on `feat/kmd-375-media-delete-provider-first` / PR #489. It makes explicit single-media deletion fail closed by requiring private provider deletion before grant revocation and `MediaAsset` tombstoning, retaining the durable storage key/retry path if provider deletion fails. It has no schema migration and must not merge until exact-head CI/runtime-readiness and review/thread gates are clear.
