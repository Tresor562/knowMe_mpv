...
- Live GitHub has progressed through merged `KMD-375`.
...
- KMD-373 exact head `3b0cc943411c6e9c472d779134e833c694eb714d` closed the account-deletion/media-upload completion race with a durable deletion marker, PostgreSQL `User` row lifecycle locking and atomic upload-session cleanup on user deletion. PR #487 merged it as `6574c8281861b7ff7ad8566df562ad0aacfa25f8` after its required validation gates passed.
- KMD-374 exact head `59cddba9e23fe0347f9ecc082958bb2abaaae252` closed the remaining upload-session-creation/account-deletion race with a PostgreSQL `BEFORE INSERT` owner/fence guard. Canonical CI #1441 and Runtime readiness #45 passed on that exact head with no submitted reviews or unresolved review threads. PR #488 merged it as squash commit `e578b3ac911b31a667fb8c4cb70ce4aa508487a3`.
- KMD-375 exact head `88f10409b667e68ea5451eb2aec6c3e28404a0d1` made explicit single-media deletion fail closed by requiring private provider deletion before grant revocation and `MediaAsset` tombstoning. Canonical CI #1444 and Runtime readiness #48 passed on that exact head with no submitted reviews or unresolved review threads. PR #489 merged it as squash commit `f89b1d4b4b6b63f54086e3c2f52e3fdcca260a1d`.
- KMD-376 is the current active delivery on `feat/kmd-376-media-grant-deletion-fence`. It serializes access/download authority with the `MediaAsset` tombstone, purges grants in the tombstone transaction and rejects authority for already-deleted assets. It includes a migration and PostgreSQL-backed E2E proof and must not merge until exact-head CI/runtime-readiness and review/thread gates are clear.
