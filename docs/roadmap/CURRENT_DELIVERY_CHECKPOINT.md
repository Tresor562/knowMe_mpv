...
- Live GitHub has progressed through merged `KMD-361`.
- Latest verified merge at reconciliation: `KMD-361`, commit `72adb40c102b81047987826780611481097a74c9`.
...
- `KMD-362` is the current runtime liveness-healthcheck candidate on `feat/kmd-362-runtime-healthchecks`.
- KMD-362 is not complete until exact-head CI proves both images retain the intended healthcheck metadata together with every existing supply-chain, migration, build, runtime and E2E gate.
...
KMD-362 adds explicit process-liveness metadata to both runtime images and a minimal non-cacheable Web liveness route. Do not claim production orchestrator health handling or alerting until deployment-specific evidence exists.
