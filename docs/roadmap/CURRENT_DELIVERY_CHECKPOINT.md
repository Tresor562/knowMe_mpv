...
- Live GitHub has progressed through merged `KMD-360`.
- Latest verified merge at reconciliation: `KMD-360`, commit `4aa4b5f4c9e231fcd3f0366c33ca061cbd8660ad`.
...
- `KMD-361` is the current runtime-container privilege hardening candidate on `feat/kmd-361-nonroot-runtime-containers`.
- KMD-361 is not complete until exact-head CI proves the effective API and Web runtime users are non-root with `NODE_ENV=production`, together with every existing quality, migration, build, container and E2E gate.
...
KMD-361 builds on that reproducibility baseline by removing UID 0 from the final API and Web runtime commands and proving the effective container identity in CI. Do not claim runtime privilege hardening until KMD-361's exact-head CI is green and the KMD is merged.
