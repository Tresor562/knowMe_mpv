...
- Live GitHub has progressed through merged `KMD-370`.
...
- KMD-371 is the current active delivery. It makes KMD-369/KMD-370 object-storage provider proof a required market-release criterion through `object_storage_provider_validation`, adds a dedicated semantic binder and action-plan mapping, and remains isolated on `feat/kmd-371-object-storage-market-evidence-binding`. It does not claim real production bucket execution and must not merge before exact-head CI plus review/thread gates are clear.
