...
- Live GitHub has progressed through merged `KMD-371`.
...
- KMD-372 is the current active delivery on `feat/kmd-372-account-media-deletion-fail-closed` / PR #486. It hardens destructive account cleanup so private object-storage deletion failures can no longer be swallowed after media metadata is removed. It must not merge until exact-head CI and review/thread gates are clear, and it does not claim a real production-provider purge.
