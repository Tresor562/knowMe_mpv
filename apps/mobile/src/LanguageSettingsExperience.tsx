      ...
      <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
        ...
        <View style={styles.actions}>
          {(['fr', 'en'] as const).map((candidate) => {
            ...
            return (
              <Pressable key={candidate} disabled={busy} onPress={() => void change(candidate)} style={({ pressed }) => [styles.button, { backgroundColor: selected ? colors.accent : colors.surfaceRaised, borderColor: colors.border }, (pressed || busy) && styles.muted]}>
                <Text style={{ color: selected ? colors.accentText : colors.text, fontWeight: '900' }}>
...
const styles = StyleSheet.create({
  ...
  button: { minWidth: 120, borderWidth: 1, borderRadius: 15, paddingVertical: 12, paddingHorizontal: 16, alignItems: 'center' },
  muted: { opacity: 0.5 },
