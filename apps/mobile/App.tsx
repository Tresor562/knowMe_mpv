...
type Screen = 'home' | 'feed' | 'challenges' | 'profile';
type User = {
  id: string;
  email: string;
  username: string;
...
function AuthScreen({ onAuthenticated }: { onAuthenticated: () => Promise<void> }) {
  ...
  const valid = password.length >= 8 && (mode === 'login' ? identifier.trim().length > 0 : displayName.trim().length > 0 && username.trim().length > 0 && email.includes('@'));
    ...
    <KeyboardAvoidingView style={styles.authRoot} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.authContent} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.card}>
          ...
          <PrimaryButton disabled={!valid || busy} onPress={submit} title={busy ? 'Chargement…' : mode === 'login' ? 'Entrer dans KnowMe' : 'Créer mon profil'} />
        </View>
      </ScrollView>
    </KeyboardAvoidingView>
  );
...
function HomeScreen({ user }: { user: User }) {
  ...
  return (
    <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); load(); }} />} contentContainerStyle={styles.screenContent}>
      ...
      <View style={styles.statsGrid}>
        ...
        <View style={styles.statCard}><Text style={styles.statValue}>{unread}</Text><Text style={styles.statLabel}>Notifications</Text></View>
      ...
      <View style={styles.card}><Text style={styles.cardTitle}>Pourquoi KnowMe ?</Text><Text style={styles.cardText}>Les interactions, les questions et les défis transforment les contacts en relations plus profondes.</Text></View>
    </ScrollView>
  );
}
...
function FeedScreen() {
  ...
  const load = useCallback(async () => {
    try { setPosts(await apiFetch<Post[]>('/posts/feed')); } finally { setRefreshing(false); }
  }, []);
  useEffect(() => { load(); }, [load]);
  ...
  async function publish() {
    ...
    try {
      ...
      Alert.alert('Publication impossible', cause instanceof Error ? cause.message : 'Réessaie plus tard.');
  ...
  async function like(postId: string) {
    try {
      ...
      setPosts((current) => current.map((post) => post.id === postId ? { ...post, _count: { ...post._count, likes: Math.max(0, post._count.likes + (result.liked ? 1 : -1)) } } : post));
...
function ChallengesScreen({ user }: { user: User }) {
  ...
  const load = useCallback(async () => setItems(await apiFetch<Challenge[]>('/challenges')), []);
  useEffect(() => { load(); }, [load]);
  ...
  async function create() {
    ...
    try {
      ...
      setTitle(''); setQuestions(''); await load();
...
function ProfileScreen({ user, onLogout, onUpdated }: { user: User; onLogout: () => Promise<void>; onUpdated: () => Promise<void> }) {
  ...
  return (
    <ScrollView contentContainerStyle={styles.screenContent}>
      ...
      <View style={styles.card}><Text style={styles.cardTitle}>Modifier mon profil</Text><Field value={displayName} onChangeText={setDisplayName} placeholder="Nom affiché" /><Field multiline value={bio} onChangeText={setBio} placeholder="Biographie" /><PrimaryButton disabled={busy || !displayName.trim()} onPress={save} title={busy ? 'Enregistrement…' : 'Enregistrer'} /></View>
...
export default function App() {
  ...
  async function logout() {
    ...
    await clearSession(); setUser(null); setScreen('home');
  }
  ...
  return (
    <SafeAreaView style={styles.root}>
      ...
      <View style={styles.tabBar}>
        {([['home', '⌂', 'Accueil'], ['feed', '◉', 'Fil'], ['challenges', '◎', 'Défis'], ['profile', '●', 'Profil']] as const).map(([value, icon, label]) => (
...
const styles = StyleSheet.create({
  root: { flex: 1, backgroundColor: '#071410' }, loadingRoot: { flex: 1, backgroundColor: '#071410', alignItems: 'center', justifyContent: 'center' }, body: { flex: 1 }, authRoot: { flex: 1, backgroundColor: '#071410' }, authContent: { flexGrow: 1, justifyContent: 'center', padding: 24 }, brandMark: { width: 64, height: 64, borderRadius: 22, backgroundColor: '#45e6bd', alignItems: 'center', justifyContent: 'center', marginBottom: 14 }, brandMarkText: { color: '#052017', fontSize: 34, fontWeight: '900' }, logo: { color: '#f4fff9', fontSize: 46, fontWeight: '900' }, subtitle: { color: '#a7b9b1', fontSize: 18, marginTop: 4, marginBottom: 24 }, screenContent: { padding: 20, paddingBottom: 36, gap: 14 }, eyebrow: { color: '#45e6bd', fontSize: 12, fontWeight: '800', letterSpacing: 1.5 }, heading: { color: '#f4fff9', fontSize: 30, fontWeight: '900', marginTop: 4 }, muted: { color: '#91a79e' }, card: { backgroundColor: '#10231d', borderColor: '#1c3a31', borderWidth: 1, borderRadius: 24, padding: 18, gap: 12 }, cardTitle: { color: '#f4fff9', fontSize: 19, fontWeight: '800' }, cardText: { color: '#b6c8c0', fontSize: 15, lineHeight: 22 }, input: { backgroundColor: '#091914', borderColor: '#25473b', borderWidth: 1, borderRadius: 16, color: '#f4fff9', fontSize: 16, paddingHorizontal: 15, paddingVertical: 13, minHeight: 50 }, primaryButton: { backgroundColor: '#45e6bd', borderRadius: 16, paddingVertical: 14, paddingHorizontal: 18, alignItems: 'center' }, primaryButtonText: { color: '#052017', fontWeight: '900', fontSize: 15 }, buttonMuted: { opacity: 0.45 }, error: { color: '#ff9d66', lineHeight: 20 }, success: { color: '#45e6bd', fontWeight: '700' }, segmented: { flexDirection: 'row', backgroundColor: '#091914', borderRadius: 14, padding: 4, marginBottom: 4 }, segment: { flex: 1, padding: 10, borderRadius: 11, alignItems: 'center' }, segmentActive: { backgroundColor: '#1b3b31' }, segmentText: { color: '#789187', fontWeight: '700' }, segmentTextActive: { color: '#f4fff9' }, statsGrid: { flexDirection: 'row', gap: 10 }, statCard: { flex: 1, backgroundColor: '#10231d', borderRadius: 20, padding: 14 }, statValue: { color: '#f4fff9', fontSize: 25, fontWeight: '900' }, statLabel: { color: '#91a79e', fontSize: 12, marginTop: 4 }, postText: { color: '#e4f2ec', fontSize: 17, lineHeight: 25 }, handle: { color: '#91a79e', marginTop: 2 }, row: { flexDirection: 'row', alignItems: 'center', gap: 12 }, smallButton: { backgroundColor: '#1b3b31', paddingHorizontal: 13, paddingVertical: 8, borderRadius: 12 }, smallButtonText: { color: '#f4fff9', fontWeight: '700' }, avatar: { width: 92, height: 92, borderRadius: 46, backgroundColor: '#45e6bd', alignItems: 'center', justifyContent: 'center' }, avatarText: { color: '#052017', fontSize: 38, fontWeight: '900' }, logoutButton: { borderColor: '#ff9d66', borderWidth: 1, borderRadius: 16, padding: 14, alignItems: 'center' }, logoutText: { color: '#ff9d66', fontWeight: '800' }, tabBar: { flexDirection: 'row', backgroundColor: '#0b1d17', borderTopColor: '#1c3a31', borderTopWidth: 1, paddingTop: 8, paddingBottom: Platform.OS === 'ios' ? 18 : 8 }, tab: { flex: 1, alignItems: 'center', gap: 2 }, tabIcon: { color: '#789187', fontSize: 20 }, tabLabel: { color: '#789187', fontSize: 11, fontWeight: '700' }, tabActive: { color: '#45e6bd' }
});
