...
export default function FeedPage() {
  ...
  return (
    <main className="shell" style={{maxWidth:760,margin:'0 auto'}}>
      <header><small style={{color:'var(--mint)'}}>ACTIVITÉ DE @{user.username}</small><h1>Fil KnowMe</h1></header>
      <form className="card" onSubmit={publish} style={{padding:18,marginBottom:18}}>
        ...
        <div style={{display:'flex',justifyContent:'flex-end',marginTop:12}}><button className="btn btn-primary" disabled={publishing}>{publishing ? 'Publication...' : 'Publier'}</button></div>
      </form>
      {message && <p role="alert" style={{color:'var(--orange)'}}>{message}</p>}
      {loading && <p>Chargement des publications...</p>}
      <section className="grid">
        {!loading && posts.length === 0 && <article className="card" style={{padding:22,textAlign:'center'}}><h2>Le fil est encore calme</h2><p style={{color:'var(--muted)'}}>Sois la première personne à partager quelque chose.</p></article>}
        {posts.map((post) => (
          <article className="card" key={post.id} style={{padding:20}}>
            <div style={{display:'flex',gap:12,alignItems:'center'}}><div style={{width:46,height:46,borderRadius:'50%',background:'var(--surface-2)',display:'grid',placeItems:'center',fontWeight:800}}>{post.author.displayName[0]}</div><div><strong>{post.author.displayName}</strong><div style={{color:'var(--muted)'}}>@{post.author.username}</div></div></div>
