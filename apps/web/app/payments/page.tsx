...
function providerName(provider: WebPaymentProvider) {
...
function ProductCard({
  ...
  providers: PaymentProviderConfiguration | null;
  busyKey: string | null;
  onCheckout: (product: CommerceProduct, price: CommercePrice) => void;
}) {
  return (
    <article
      className="card"
    ...
    >
      <div>
        ...
        <p style={{ color: 'var(--muted)', margin: 0 }}>
          {product.description ?? 'Produit KnowMe délivré après vérification serveur.'}
        </p>
      </div>
      ...
      <div className="grid" style={{ gap: 10 }}>
        {product.prices.map((price) => {
          const provider = price.provider as WebPaymentProvider;
          const configured = Boolean(providers?.providers[provider]?.configured);
          const key = `${product.key}:${price.id}`;
          return (
            <div
              key={price.id}
              style={{
            ...
            >
              ...
              >
                {busyKey === key
                  ? 'Création…'
                  : configured
                    ? `Payer avec ${providerName(provider)}`
                    : `${providerName(provider)} indisponible`}
              </button>
            </div>
          );
        })}
...
export default function PaymentsPage() {
  ...
  const loadCatalog = useCallback(async () => {
    ...
    try {
      ...
      setMessage(errorMessage(cause, 'Le catalogue de paiement est indisponible.'));
    } finally {
  ...
  const loadOrders = useCallback(async () => {
    ...
    try {
      ...
      setMessage(errorMessage(cause, 'L’historique des paiements est indisponible.'));
    } finally {
  ...
  const configuredProviders = useMemo(
    () =>
      providers
        ? (Object.entries(providers.providers) as Array<[
            keyof PaymentProviderConfiguration['providers'],
            { configured: boolean; platform: string }
  ...
  async function checkout(product: CommerceProduct, price: CommercePrice) {
    if (!user || (price.provider !== 'FLUTTERWAVE' && price.provider !== 'CINETPAY')) return;
    const key = `${product.key}:${price.id}`;
    setBusyKey(key);
    setMessage('');
    try {
      ...
      const result = await createWebCheckout(
        {
          ...
          provider,
          countryCode: price.countryCode ?? normalizeCountry(country) || undefined,
          currency: price.currency,
          phoneNumber: phoneNumber.trim() || undefined,
          address: address.trim() || undefined,
          city: city.trim() || undefined,
          customerCountryCode: normalizeCountry(country) || undefined,
      ...
      setMessage(errorMessage(cause, 'Impossible de créer le paiement.'));
      setBusyKey(null);
      await loadOrders();
    }
  }
    ...
    <main className="shell" style={{ maxWidth: 1180, margin: '0 auto' }}>
      <header style={{ marginBottom: 24 }}>
        ...
        <p style={{ color: 'var(--muted)', maxWidth: 820 }}>
          Les prix, produits, statuts et attributions sont décidés par le serveur. Le navigateur ne
          transmet jamais un montant libre et un achat n’est délivré qu’après vérification du fournisseur.
        </p>
      </header>
      ...
      <section className="card" style={{ padding: 22, marginBottom: 22 }}>
        ...
        <p style={{ color: 'var(--muted)', fontSize: 13, marginBottom: 0 }}>
          Fournisseurs actifs : {configuredProviders.length > 0
          ...
          >
            {catalog.map((product) => (
              <ProductCard
                ...
                providers={providers}
                busyKey={busyKey}
                onCheckout={(entry, price) => void checkout(entry, price)}
              />
            ))}
          </div>
        )}
