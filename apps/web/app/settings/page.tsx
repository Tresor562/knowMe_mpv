  ...
  return (
    <main className="shell" style={{ maxWidth: 820, margin: '0 auto' }}>
      <header>
        ...
        <p style={{ color: 'var(--muted)' }}>Compte de {user.displayName} (@{user.username})</p>
      </header>
      {message && <p role="alert" style={{ color: 'var(--orange)' }}>{message}</p>}

      <section className="card" style={{ padding: 24, marginBottom: 20 }}>
        <h2>{t('settings.languageTitle')}</h2>
        <p style={{ color: 'var(--muted)', lineHeight: 1.6 }}>{t('settings.languageDescription')}</p>
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          <button className={locale === 'fr' ? 'btn btn-primary' : 'btn'} disabled={languageBusy} onClick={() => void changeLanguage('fr')}>{t('common.french')}</button>
          <button className={locale === 'en' ? 'btn btn-primary' : 'btn'} disabled={languageBusy} onClick={() => void changeLanguage('en')}>{t('common.english')}</button>
        </div>
        <p style={{ color: 'var(--muted)', marginBottom: 0 }}>
          {t('settings.languageFallback')} · v{version}{persisted ? '' : ' · détectée sur cet appareil'}
        </p>
      </section>
      ...
      <section className="card" style={{ padding: 24, marginBottom: 20 }}>
        ...
        <p style={{ color: 'var(--muted)', lineHeight: 1.6 }}>Télécharge une copie JSON de ton profil et de tes données KnowMe.</p>
      ...
      <section className="card" style={{ padding: 24, border: '1px solid rgba(255,120,80,.35)' }}>
        ...
        <p style={{ color: 'var(--muted)', lineHeight: 1.6 }}>La suppression est définitive. Tes sessions et tes données seront supprimées selon les règles du service.</p>
