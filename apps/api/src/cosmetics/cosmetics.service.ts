...
export class CosmeticsService {
  ...
  async equip(userId:string,slot:string,dto:EquipCosmeticDto){
    ...
    if(current?.itemId===dto.itemId){
      const ownership=await this.prisma.cosmeticOwnership.findUnique({where:{userId_itemId:{userId,itemId:dto.itemId}}});
      if(!ownership||ownership.revokedAt) throw new ForbiddenException('Cet objet ne fait pas partie de ton inventaire.');
      if(!this.slotMatches(current.item.slot,slot)) throw new BadRequestException('Cet objet ne correspond pas à cet emplacement.');
      if(!this.isAvailable(current.item)) throw new BadRequestException('Cet objet cosmétique n’est pas actuellement disponible.');
      this.assertAvatarRuntimeReady(current.item);
