...
export type AvatarRuntimeComposition = {
  dna: AvatarDNA;
  renderProfile: AvatarRenderProfile;
  baseBody: AvatarAssetManifest;
  equipped: AvatarAssetManifest[];
};

export type AvatarRuntimeCompositionMetrics = {
...
function requiredMorphs(asset: AvatarAssetManifest, names: readonly string[], label: string) {
  for (const name of names) if (!asset.morphTargets.includes(name)) throw new Error(`${label} cannot follow Avatar DNA control ${name}`);
}

function canonicalAsset(value: AvatarAssetManifest, label: string) {
  try { return validateAvatarAssetManifest(value); }
  catch (error) { throw new Error(`${label} is not runtime-certified: ${error instanceof Error ? error.message : 'invalid asset'}`); }
}

/**
 * Certifies the assembled avatar, not merely each asset in isolation.
 * This is the final runtime gate before an Android renderer may consume a set
 * of GLBs. Commerce/ownership remains authoritative elsewhere; this contract
 * proves geometry/rig/morph/render compatibility only.
 */
export function validateAvatarRuntimeComposition(value: AvatarRuntimeComposition, lodLevel: 0 | 1 | 2 = 0): CertifiedAvatarRuntimeComposition {
  const dna = validateAvatarDNA(value.dna);
  const renderProfile = validateAvatarRenderProfile(value.renderProfile);
  ...
  const equipped = value.equipped.map((asset, index) => canonicalAsset(asset, `Equipped asset ${index}`));
  const slots = new Set<string>();
  for (const asset of equipped) {
    if (!asset.slot) throw new Error('Equipped runtime assets require a Cosmetics slot');
    if (slots.has(asset.slot)) throw new Error(`Runtime composition contains multiple assets for slot ${asset.slot}`);
  ...
  // One conservative geometry draw plus one additional draw for each declared
  // optional PBR channel. This is intentionally an upper-bound admission check;
  // measured GPU profiling remains part of artistic certification.
  const estimatedDrawCalls = all.reduce((sum, asset) => sum + 1 + ['normal','metallicRoughness','occlusion','emissive'].filter((key) => asset.textures[key as keyof typeof asset.textures]).length, 0);
