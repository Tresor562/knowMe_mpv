...
const MANIFEST_KEYS=new Set(['manifestVersion','clipKey','kind','uri','durationMs','loop','skeletonKey','facialRigKey','drivenMorphTargets','sampleRate']);
...
function assertRuntimeUri(uri:string){if(typeof uri!=='string'||!uri.startsWith('asset://runtime/avatar/animations/')||uri.includes('..')||uri.includes('?')||uri.includes('#'))throw new Error('Avatar animation URI must be an internal certified runtime animation asset.');}

/** Validates untrusted animation metadata and returns a detached canonical projection. */
export function validateAvatarAnimationManifest(input:AvatarAnimationManifest):AvatarAnimationManifest{
  ...
  const result:AvatarAnimationManifest={manifestVersion:AVATAR_ANIMATION_MANIFEST_VERSION,clipKey:input.clipKey,kind:input.kind,uri:input.uri,durationMs:input.durationMs,loop:input.loop,drivenMorphTargets:[...input.drivenMorphTargets],sampleRate:input.sampleRate};
