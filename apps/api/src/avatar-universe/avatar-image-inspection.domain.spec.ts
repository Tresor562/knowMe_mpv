...
describe('Avatar embedded texture inspection',()=>{
 ...
 it('reads KTX2 dimensions and mip count from the canonical 80-byte header',()=>{const x=inspectAvatarEmbeddedImage(ktx2(),'image/ktx2');expect(x.width).toBe(1024);expect(x.height).toBe(1024);expect(x.mipLevels).toBe(11);expect(x.estimatedRgba8GpuBytes).toBeGreaterThan(1024*1024*4);});
 ...
 it('rejects truncated KTX2 level indices',()=>{const b=ktx2(1024,1024,11).subarray(0,80+10*24);expect(()=>inspectAvatarEmbeddedImage(b,'image/ktx2')).toThrow(/level index/);});
