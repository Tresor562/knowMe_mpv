...
export const HERO_ANDROID_MAX_COLD_LOAD_MS=2500;
export const HERO_ANDROID_MAX_WARM_LOAD_MS=900;
export const HERO_ANDROID_MAX_P95_FRAME_MS=33.34;
export const HERO_ANDROID_MAX_P99_FRAME_MS=50;
export const HERO_ANDROID_MIN_SAMPLE_FRAMES=600;
...
function finiteNonNegative(name:string,value:number){if(!Number.isFinite(value)||value<0)throw new Error(`Android profile ${name} is invalid.`);}
function nonEmpty(name:string,value:string){if(typeof value!=='string'||value.trim().length===0)throw new Error(`Android profile ${name} is required.`);}
...
export function verifyHeroAndroidMeasuredProfile(profile:HeroAndroidMeasuredProfile){
 ...
 for(const [name,value] of Object.entries({coldLoadMs:m?.coldLoadMs,warmLoadMs:m?.warmLoadMs,p95FrameMs:m?.p95FrameMs,p99FrameMs:m?.p99FrameMs,peakJavaBytes:m?.peakJavaBytes,peakNativeBytes:m?.peakNativeBytes,peakGpuBytes:m?.peakGpuBytes}))finiteNonNegative(name,value as number);
 ...
 if(m.warmLoadMs>m.coldLoadMs)throw new Error('Android warm load cannot exceed cold load in a certified run.');
 if(m.p95FrameMs>m.p99FrameMs)throw new Error('Android p95 frame time cannot exceed p99.');
 if(m.coldLoadMs>HERO_ANDROID_MAX_COLD_LOAD_MS)throw new Error('Android cold-load budget exceeded.');
 if(m.warmLoadMs>HERO_ANDROID_MAX_WARM_LOAD_MS)throw new Error('Android warm-load budget exceeded.');
 if(m.p95FrameMs>HERO_ANDROID_MAX_P95_FRAME_MS)throw new Error('Android p95 frame-time budget exceeded.');
 if(m.p99FrameMs>HERO_ANDROID_MAX_P99_FRAME_MS)throw new Error('Android p99 frame-time budget exceeded.');
...
export function certifyHeroRuntimeAndroidMeasured(bundle:HeroRuntimeBundle,files:ReadonlyMap<string,Uint8Array>,profile:HeroAndroidMeasuredProfile){
 const staticCertification=verifyHeroRuntimeBundleAndroidBytes(bundle,files);
 const measuredProfile=verifyHeroAndroidMeasuredProfile(profile);
