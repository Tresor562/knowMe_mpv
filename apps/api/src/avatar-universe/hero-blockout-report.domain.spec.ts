...
describe('Hero blockout production gate',()=>{
 ...
 it('rejects a syntactically valid but non-canonical asset key',()=>{const x=valid();(x as unknown as Record<string,unknown>).assetKey='knowme.hero.attacker.v1';expect(()=>validateHeroBlockoutReport(x)).toThrow(/canonical Hero source/);});
 ...
 it('rejects an A-pose label without explicit measured pose evidence',()=>{const x=valid();x.poseVerified=false as true;expect(()=>validateHeroBlockoutReport(x)).toThrow(/arm-angle evidence/);});
 it('rejects T-pose-like measured arm angles',()=>{const x=valid();x.measuredLeftUpperArmAngleDeg=0;x.measuredRightUpperArmAngleDeg=0;expect(()=>validateHeroBlockoutReport(x)).toThrow(/canonical A-pose/);});
 it('rejects down-arm-like measured angles',()=>{const x=valid();x.measuredLeftUpperArmAngleDeg=85;x.measuredRightUpperArmAngleDeg=85;expect(()=>validateHeroBlockoutReport(x)).toThrow(/canonical A-pose/);});
 it('rejects asymmetric measured A-pose evidence',()=>{const x=valid();x.measuredLeftUpperArmAngleDeg=30;x.measuredRightUpperArmAngleDeg=50;expect(()=>validateHeroBlockoutReport(x)).toThrow(/canonical A-pose/);});
 it('rejects malformed measured A-pose numbers',()=>{const x=valid();(x as unknown as Record<string,unknown>).measuredLeftUpperArmAngleDeg='40';expect(()=>validateHeroBlockoutReport(x)).toThrow(/arm-angle evidence/);});
