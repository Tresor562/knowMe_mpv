...
export function validateAvatarGlbInspection(manifest:AvatarAssetManifest,lod:AvatarLod,inspection:AvatarGlbInspection){
 ...
 // During parser rollout, explicit negative audit results are enforced immediately while
 // legacy parser fixtures may omit the new proof fields. Once parser wiring lands, these
 // checks become strict === true gates without breaking unrelated certification fixtures.
 if(manifest.geometry?.skinned){if(inspection.rigValid===false)throw new Error(`Skinned avatar GLB has an invalid rig${inspection.rigIssues?.length?`: ${inspection.rigIssues.join(' ')}`:'.'}`);if(inspection.skins<1)throw new Error('Skinned avatar GLB must contain a skin.');if(inspection.maxBonesPerVertex>4)throw new Error('Avatar GLB exceeds 4 non-zero bone influences per vertex.');if(inspection.joints.length===0)throw new Error('Skinned avatar GLB must expose joints.');if(!inspection.normalizedSkinWeights)throw new Error('Avatar GLB skin weights must be normalized per vertex.');}
 if(inspection.animations>0&&inspection.animationsValid===false)throw new Error(`Runtime avatar GLB has invalid animations${inspection.animationIssues?.length?`: ${inspection.animationIssues.join(' ')}`:'.'}`);
