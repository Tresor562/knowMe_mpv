...
export function validateHeroBlockoutReport(input:HeroBlockoutReport):HeroBlockoutReport{
 ...
 if(input.groundContactVerified!==true||typeof input.measuredGroundContactMeters!=='number'||!Number.isFinite(input.measuredGroundContactMeters)||Math.abs(input.measuredGroundContactMeters)>HERO_BLOCKOUT_BUDGETS.maxGroundOffsetMeters)throw new Error('Hero blockout ground contact must be measured within tolerance.');
 if(typeof input.measuredBodyCenterX!=='number'||!Number.isFinite(input.measuredBodyCenterX)||Math.abs(input.measuredBodyCenterX)>HERO_BLOCKOUT_BUDGETS.maxCenterOffsetMeters)throw new Error('Hero blockout body must be measurably centered on the world X axis.');
 if(input.skeletonTarget!==AVATAR_CANONICAL_SKELETON)throw new Error('Hero blockout must target the canonical skeleton.');
 if(typeof input.bodyHeightMeters!=='number'||!Number.isFinite(input.bodyHeightMeters)||input.bodyHeightMeters<HERO_BLOCKOUT_BUDGETS.minHeightMeters||input.bodyHeightMeters>HERO_BLOCKOUT_BUDGETS.maxHeightMeters)throw new Error('Hero blockout body height is outside the supported human range.');
