...
export const HERO_BLOCKOUT_REPORT_VERSION = 7 as const;
...
export function validateHeroBlockoutReport(input:HeroBlockoutReport):HeroBlockoutReport{
 ...
 if(input.texturesVerified!==true||input.bodyBaseColorTextureVerified!==true||!isSafeNonNegativeInteger(input.measuredTextureCount)||input.measuredTextureCount<1||!isSafeNonNegativeInteger(input.measuredMaxTextureDimension)||input.measuredMaxTextureDimension<1||input.measuredMaxTextureDimension>HERO_BLOCKOUT_BUDGETS.maxTextureDimension||!isSafeNonNegativeInteger(input.measuredTotalTexturePixels)||input.measuredTotalTexturePixels<1||input.measuredTotalTexturePixels>HERO_BLOCKOUT_BUDGETS.maxTotalTexturePixels)throw new Error('Hero texture evidence is missing or outside the Android texture budget.');
