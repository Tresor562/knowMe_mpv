...
export class ProfileCircleNotificationOperationalAlertService {
  ...
  async refresh(now = new Date()) {
    ...
    const candidates = [
      openCircuits > 0
        ? {
            ...
            severity: openCircuits >= 3 ? ('CRITICAL' as const) : ('HIGH' as const),
      ...
      openDeadLetters > 0
        ? {
            ...
            severity: openDeadLetters >= 100 ? ('HIGH' as const) : ('NORMAL' as const),
      ...
      failedAttempts >= 50
        ? {
            ...
            severity: failedAttempts >= 500 ? ('CRITICAL' as const) : ('HIGH' as const),
    ...
    return { openCircuits, openDeadLetters, failedAttempts, active: candidates.length };
  }

  list() {
    return this.prisma.profileCircleNotificationOperationalAlert.findMany({
      orderBy: [{ active: 'desc' }, { severity: 'desc' }, { lastSeenAt: 'desc' }],
      take: 200
    });
  }
}
