...
export class ProfileCircleNotificationRetryPlannerService {
  ...
  async releaseDue(input: { limit?: number; now?: Date } = {}) {
    const now = input.now ?? new Date();
    const due = await this.prisma.profileCircleNotificationTransportAttempt.findMany({
  ...
  private priorityFromMetadata(metadata: unknown): ProfileCircleNotificationPriority {
    if (
      metadata &&
