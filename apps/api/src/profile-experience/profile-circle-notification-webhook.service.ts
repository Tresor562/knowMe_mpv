...
class ProfileCircleNotificationWebhookDto {
  ...
  @IsIn(['SENT', 'DELIVERED', 'TEMPORARY_FAILURE', 'PERMANENT_FAILURE', 'COMPLAINT'])
  outcome!:
    | 'SENT'
    | 'DELIVERED'
    | 'TEMPORARY_FAILURE'
...
function canonicalPayload(provider: string, dto: ProfileCircleNotificationWebhookDto) {
...
export class ProfileCircleNotificationWebhookService {
  ...
  async process(input: {
    ...
    const secret = this.config
      .get<string>(`PROFILE_NOTIFICATION_${provider.toUpperCase()}_WEBHOOK_SECRET`)
      ?.trim();
    ...
    const receipt =
      await this.prisma.profileCircleNotificationWebhookReceipt.upsert({
        where: { provider_eventId: { provider, eventId: input.dto.eventId } },
        create: {
          provider,
